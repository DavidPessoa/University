...
-- Barra shoot
-----------------------------------------------------------------------------------------
local sheetData =  { width = 199 , height= 50, count = 1 , numFrames = 26 }
local spriteSheet = graphics.newImageSheet("imagens/Barra completa2.png" , sheetData)
 
local sequenceData = { 
  {name = "shoot", start =1,  count=26, time=500, loopCount = 0 } }
  barra_completa = display.newSprite (spriteSheet, sequenceData) 
  barra_completa.x=370
  barra_completa.y=280
------------------------------------------------------------------------------------------
-- Mute button
------------------------------------------------------------------------------------------
local function mute( event )
    ...
    if "began" == phase then
            ...
            composer.removeScene("modoBarra") 
...
-- Frame 1-------------------------------------------------------------------------------
local function frame1(e)
  barra_completa:setFrame(1)
end
-----------------------------------------------------------------------------------------
-- Shoot System
-----------------------------------------------------------------------------------------
...
local function shoot( event )
    ...
    barra_completa:setSequence("shoot")      
        barra_completa:play()
        ...
        if "ended" == phase or "cancelled" == phase then
            ...
            barra_completa:pause() 
            local frameshot= barra_completa.frame
            if frameshot <= 5 or frameshot>=23 then
               ball:applyLinearImpulse(0.3,1.5,ball.x,ball.y)
            elseif (frameshot > 5 and frameshot < 9) or (frameshot>=20 and frameshot<23)  then 
               ball:applyLinearImpulse(0.7,1.5,ball.x,ball.y)              
            elseif frameshot >= 9 and frameshot<20 then
               ball:applyLinearImpulse(1,1.5,ball.x,ball.y)                       
            end
            timer.performWithDelay(2500, returnball, 1)
            timer.performWithDelay(2500, frame1, 1)            
        end
    end
 
    return true
end
ball:addEventListener("touch", shoot)
-----------------------------------------------------------------------------------------
-- Score System 
-----------------------------------------------------------------------------------------
scoreCtr = 0
